...
namespace ACdb
{
    ...
    {
        public static HashSet<BaseItem> FetchImages = new HashSet<BaseItem>();
        ...
        public static void Initialize(ILibraryManager libraryManager, IDirectoryService directoryService, ITaskManager taskManager, ICollectionManager collectionManager, IFileSystem fileSystem, IUserManager userManager)
        {
            ...
            collectionManager.ItemsRemovedFromCollection += (sender, args) =>
            {
                ...
                {
                    SettingsManager.CollectionRemovedCleanup(args.Collection.Id); // todo this is not called on jellyfin delete on server, I could just do a settings clean up after every sync?
                }
            };
        ...
        private static int? lastRunDuration;
        public static int? LastRanDurationSec
        {
            get { return lastRunDuration; }
            set { lastRunDuration = value; }
        }
        ...
        public static string ApiKey
        {
            ...
            set
            {
                ...
                if (SettingsManager.GetApiKey() != _apiKey)
                {
                    ...
                    if (_schedulingManager.HasTriggers() == false)
                    {
        ...
        {
            _schedulingManager.ResetToDefault(ScheduleIntervalMinutes);
        }
        ...
        public static string TimeSinceJobLastRan()
        {
            ...
            return $"{Utils.HumanReadablerDuration(lastRan ?? DateTime.MinValue, DateTime.Now)} ago";
        }
        ...
        public static string TimeUntilNextSync()
        {
            ...
            return $"Approx. {Utils.ConvertSecondsToHumanReadable(seconds.Value)}";
        }
...
#pragma warning restore IDE0060
        {
            ...
            string jobUrl = $"{PluginConfig.ApiGetJobsUrl}?v={PluginConfig.PluginVersion}&plugin-type={PluginConfig.PluginType}&cv={PluginConfig.ClientVersion}";
            string json = await _api.Get(ApiKey, jobUrl, CancellationToken.None);

            if (json is null || json == string.Empty)
            {
                LogManager.Error($"Got an invalid response from {PluginConfig.WebSiteUrl}.", new ActivityLogEventArgs { Progress = 100, Description = json });
                return;
            }
            ...
            {
                LogManager.Error($"Error processing json from {PluginConfig.WebSiteUrl}.", new ActivityLogEventArgs { Progress = 100, Description = e.Message });
            ...
            ProcessCollections processCollections = new ProcessCollections(_libraryManager, response.collections_sync, progress, currentProgress);
            await processCollections.ProcessJobsAsync();
